...
{
    /**
     * comparator用于维护TreeMap中的顺序，如果使用键的自然顺序，则为null。
     * 
     * @serial
     */
    ...
    /**
     * 使用key的自然顺序构造一个新的空TreeMap。插入到Map中的所有key必须实现Comparable接口。
     * 而且所有这些key必须是相互可比的,如果用户试图将一个key放入违反此约束的Map中那么put调用将抛出一个ClassCastException。
     */
    public TreeMap()
    {
        comparator = null;
    ...
    /**
     * 构造一个新的空TreeMap。使用提供的comparator进行排序，所有这些key使用compare比较必须是相互可比的,
     * 如果用户试图将一个key放入违反此约束的Map中那么put调用将抛出一个ClassCastException。
     */
    ...
    /**
     * 构造一个新的TreeMap，其中包含与给定Map相同的Map，按照key的自然顺序排序。插入到Map中的所有key必须实现Comparable接口。
     * 而且所有这些key必须是相互可比的,如果用户试图将一个key放入违反此约束的Map中那么put调用将抛出一个ClassCastException。
     */
    ...
    /**
     * 构造一个包含相同Map的新TreeMap，并使用与指定SortedMap相同的顺序。这种方法在线性时间内运行。
     */
    ...
    /**
     * 如果此Map将一个或多个键映射到指定的值，则返回true。
     */
    public boolean containsValue(Object value)
    {
        // getFirstkey 返回红黑树最左下的值而不是root
     ...
     * 将指定Map的所有映射复制到此Map，指定Map中和此Map中相同的key会被替换（这个意思？）
     */
    public void putAll(Map<? extends K, ? extends V> map)
    {
        int mapSize = map.size();
        // 如果当前是null的treeMap，参数map又是shortMap，
    ...
    /**
     * 获取与指定key对应的项;如果不存在这样的项，则返回大于指定key的最小key的项;如果不存在这样的条目(即，树中最大的key值小于指定的key值)，返回null。
     */
    ...
    /**
     * 获取与指定键对应的项;如果不存在这样的项，则返回小于指定键的最大键的项;如果不存在这样的条目，则返回null。
     */
    ...
    /**
     * 获取大于指定键的最小键的项;如果不存在这样的项，则返回大于指定键的最小键的项;如果不存在这样的条目，则返回null。
     */
    ...
    /**
     * 返回小于指定键的最大键的项;如果不存在这样的条目(即，则树中最小的键值大于指定的键值)，返回null。
     */
    ...
    /**
     * 在第一次请求该视图时，初始化为包含条目集视图实例的字段。视图是无状态的，因此没有理由创建多个视图。
     */
    ...
    /**
     * 返回该映射中键值范围从fromKey到toKey的部分的视图。如果fromKey和toKey相等，则返回的映射为空，除非fromInclusive和toInclusive都为true。返回的映射得到此映射的支持，因此返回映射中的更改将反映在此映射中，反之亦然。返回的映射支持此映射支持的所有可选映射操作。
     */
    ...
    /**
     * 返回映射中键值小于(或等于，如果包含为真)toKey部分的视图。返回的映射得到此映射的支持，因此返回映射中的更改将反映在此映射中，反之亦然。返回的映射支持此映射支持的所有可选映射操作。
    ...
    /**
     * 返回映射中键值大于(或等于，如果为真)fromKey的部分的视图。返回的映射得到此映射的支持，因此返回映射中的更改将反映在此映射中，反之亦然。返回的映射支持此映射支持的所有可选映射操作。
     * 返回的映射将在试图插入超出其范围的键时抛出IllegalArgumentException。
     */
    ...
    /**
     * 返回一个SimpleImmutableEntry类型(不可变的final类型)的entry
     */
    ...
    /**
     * 返回当前节点的后继节点，简单说就是返回比当前节点大的节点中的最小节点
     */
    ...
    /**
     * Balancing operations. Implementations of rebalancings during insertion
     * and deletion are slightly different than the CLR version. Rather than
     * using dummy nilnodes, we use a set of accessors that deal properly with
     * null. They are used to avoid messiness surrounding nullness checks in the
     * main algorithms.
     * 平衡操作。插入和删除期间的重新平衡实现与CLR版本略有不同。我们使用一组正确处理null的访问器，而不是使用伪nilnodes。在主要的算法中，它们被用来避免围绕空值检查的混乱。
     */
     ...
     * == null). 4) A stream of serialized keys. (it == null, defaultVal !=
     * null). It is assumed that the comparator of the TreeMap is already set
     ...
     *            from this iterator.
     * @param str If non-null, new entries are created from keys and possibly
     ...
     *            in the map. If null, each value is read from iterator or
            ...
            java.io.ObjectInputStream str, V defaultVal) throws java.io.IOException, ClassNotFoundException
    {
         ...
         * all of its elements. We can then proceed with right subtree. The lo
         * and hi arguments are the minimum and maximum indices to pull out of
         * the iterator or stream for current subtree. They are not actually
         * indexed, we just proceed sequentially, ensuring that items are
         * extracted in corresponding order.
         */
    ...
    /**
     * Currently, we support Spliterator-based versions only for the full map,
     * in either plain of descending form, otherwise relying on defaults because
     * size estimation for submaps would dominate costs. The type tests needed
     * to check these for key views are not very nice but avoid disrupting
     * existing class structures. Callers must use plain default spliterators if
     * this returns null.
     */
    ...
    /**
     * Base class for spliterators. Iteration starts at a given origin and
     * continues up to but not including a given fence (or null for end). At
     * top-level, for ascending cases, the first split uses the root as
     * left-fence/right-origin. From there, right-hand splits replace the
     * current fence with its left child, also serving as origin for the
     * split-off spliterator. Left-hands are symmetric. Descending versions
     * place the origin at the end and invert ascending split rules. This base
     * class is non-commital about directionality, or whether the top-level
     * spliterator covers the whole tree. This means that the actual split
     * mechanics are located in subclasses. Some of the subclass trySplit
     * methods are identical (except for return types), but not nicely
     * factorable. Currently, subclass versions exist only for the full map
     * (including descending keys via its descendingMap). Others are possible
     * but currently not worthwhile because submaps require O(n) computations to
     * determine size, which substantially limits potential speed-ups of using
     * custom Spliterators versus default mechanics. To boostrap initialization,
     * external constructors use negative size estimates: -1 for ascend, -2 for
     * descend.
     */
